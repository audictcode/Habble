...
class UserResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Gestionar Usuarios';
            ...
            ->schema([
                Grid::make(['default' => 0])->schema([
                    ...
                    Forms\Components\TextInput::make('topics_comment_count')
                        ->label('Total de Comentarios no Foro')
                        ->numeric()
                        ->disabled(),

                    Forms\Components\Toggle::make('disabled')
                        ->label('Cuenta desactivada')
                        ->helperText('Marca arriba para desactivar una cuenta, es preferible no eliminarla para no perder sus datos.'),

                    Forms\Components\Select::make('rank')
                        ->label('Rango')
                        ->options(User::rankOptions())
                        ->default(1)
                        ->required(),
                ])
            ]);
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('rank')
                    ->label('Rango')
                    ->formatStateUsing(fn ($state) => User::RANK_LABELS[(int) $state] ?? ('Rango ' . (int) $state))
                    ->sortable(),
